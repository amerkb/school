    ...
    <div class="card">
        ...
        <div class="card-body">
            <ul class="nav nav-tabs nav-tabs-highlight">
                <li class="nav-item"><a href="#add-tt" class="nav-link active" data-toggle="tab">Edit Lecture</a></li>
            </ul>
            ...
            <div class="tab-content">
                <div class="tab-pane fade show active" id="add-tt">
                    <div class="col-md-8">
                        <form class="ajax-store" method="post" action="{{route('l.update',$lecture->id)}}">
                            @csrf
                                ...
                                <div class="col-lg-9">
                                    <select class="select form-control" required style="height: 50px" name="subject_id">
                                        ...
                                        @foreach($subjects as $subject)
                                            <option {{ $subject-> id == $lecture->subject_id ? 'selected' : '' }}  value="{{ $subject->id }}">{{ $subject->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                                ...
                                <div class="col-lg-9">
                                    <select required data-placeholder="Select Class"style="height: 50px" class="form-control select" name="teacher_id" id="teacher_id">
                                        ...
                                        @foreach($teachers as $teacher)
                                            <option {{ $teacher-> id == $lecture->teacher_id ? 'selected' : '' }} value="{{ $teacher->id }}">{{ $teacher->Name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                            <div class="form-group row">
                                ...
                                <div class="col-lg-9">
                                    <select required data-placeholder="Select Class"style="height: 50px" class="form-control select" name="day_id" id="day_id">
                                        <option selected disabled>{{('Choose')}}...</option>
                                        @foreach($days as $day)
                                            <option {{ $day-> id == $lecture->day_id ? 'selected' : '' }}  value="{{ $day->id }}">{{ $day->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                            <div class="form-group row">
                                ...
                                <div class="col-lg-9">
                                    ...
                                    <select required data-placeholder="Select Class"style="height: 50px" class="form-control select" name="ts_id" id="ts_id">
                                        <option selected disabled>{{('Choose')}}...</option>
                                        @foreach($ts as $ts)
                                            <option {{ $ts-> id == $lecture->ts_id ? 'selected' : '' }}  value="{{ $ts->id }}">{{ $ts->full }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
